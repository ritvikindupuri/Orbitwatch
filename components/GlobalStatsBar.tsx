// Import React to fix 'Cannot find namespace React' errors for React.FC
import React, { useMemo, useState, useEffect } from 'react';
import { AnomalyAlert } from '../types.ts';
import { RiskLevel } from '../constants.tsx';
...
export const GlobalStatsBar: React.FC<GlobalStatsBarProps> = ({ alerts, satelliteCount, lastSyncTime, isSyncing, onSync }) => {
    const [elasticStatus, setElasticStatus] = useState<'CONNECTED' | 'OFFLINE' | 'UNAUTHORIZED'>(relayService.getElasticStatus());

    useEffect(() => {
        const i = setInterval(() => {
            setElasticStatus(relayService.getElasticStatus());
        }, 2000);
        ...
        <div className="px-3 py-4 bg-black border-b border-white/10 z-10">
            <div className="flex items-center space-x-3">
                ...
                <StatCard 
                    title="ELASTIC LINK" 
                    value={elasticStatus} 
                    valueColor={elasticStatus === 'CONNECTED' ? 'text-green-500' : elasticStatus === 'UNAUTHORIZED' ? 'text-orange-500' : 'text-red-500'} 
                    tooltipTitle="Intelligence Relay Protocol"
                    tooltipContent="Status of the connection to the global intelligence database."
                    tooltipFooter={elasticStatus === 'CONNECTED' ? "Connection Active" : elasticStatus === 'UNAUTHORIZED' ? "Invalid Credentials" : "Link Down"}
                />
                <StatCard 
                    title="ANOMALY FEED" 
                    value={activeAlerts} 
