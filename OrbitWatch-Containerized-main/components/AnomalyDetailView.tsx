...
export const AnomalyDetailView: React.FC<AnomalyDetailViewProps> = ({ alert, onBack, onArchive, onSaveNotes, commitState = 'idle', commitError }) => {
    ...
    // Step 3 Validation: 100 time steps as per technical spec
        ...
        <div className="flex flex-col h-full bg-black font-sans border-l border-white/10 overflow-hidden relative">
                                            ...
                                            <RechartsTooltip 
                                                ...
                                                content={({ active, payload }) => {
                                                    if (active && payload && payload.length) {
                                                        ...
                                                        return (
                                                            <div className="bg-black/95 border border-white/20 p-4 font-mono shadow-2xl backdrop-blur-3xl min-w-[200px] ring-1 ring-white/10">
                                                                ...
                                                                <div className="space-y-2">
                                                                    ...
                                                                    <div className="flex justify-between text-[11px] pt-3 border-t border-white/5 mt-2">
                                                                        ...
                                                                        <span className={p.isJamming ? "text-red-500 font-black animate-pulse" : "text-green-500 font-black"}>
                                                                            {p.isJamming ? "ACTIVE DETECTION" : "NOMINAL CARRIER"}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                                {p.isJamming && (
                                                                    <div className="mt-4 pt-2 bg-red-950/20 border-t border-red-500/30">
                                                                        <p className="text-[9px] text-red-400 uppercase font-black tracking-tighter text-center">Spectral Noise Injection Detected</p>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        );
            ...
            <div className="shrink-0 p-6 bg-black border-t border-white/10 flex flex-col gap-3 z-40 relative shadow-[0_-20px_60px_rgba(0,0,0,0.9)]">
                <button
                    ...
                    className={`w-full py-4 text-[11px] font-black uppercase tracking-[0.4em] rounded-sm transition-all font-display ${
                        commitState === 'committing'
                            ? 'bg-white/20 text-white/50 cursor-wait'
                            : commitState === 'success'
                            ? 'bg-cyan-400 text-black'
                            : commitState === 'error'
                            ? 'bg-orange-900/30 text-orange-400 border border-orange-500/40'
                            : 'bg-white text-black hover:bg-cyan-400 active:scale-[0.98]'
                    }`}
                >
                    {commitState === 'committing' && '▸ COMMITTING TO LEDGER...'}
                    {commitState === 'success'    && '✓ COMMITTED — OPENING DOSSIER'}
                    {commitState === 'error'      && '⚠ RELAY OFFLINE — CASE QUEUED'}
                    {commitState === 'idle'       && 'COMMIT TO FORENSIC LEDGER'}
                </button>
                {commitState === 'error' && commitError && (
                    <p className="text-[9px] text-orange-400/70 font-mono uppercase tracking-widest text-center -mt-1">{commitError}</p>
                )}
