...
const FilterControl: React.FC<{
    ...
    options: string[];
    disabled: boolean;
}> = ({ label, defaultLabel, value, onChange, options, disabled }) => (
        ...
        <div className="flex-1 bg-black flex flex-col h-full overflow-hidden">
            ...
            <div className="p-6 border-b border-white/10 space-y-6 shrink-0 bg-white/[0.02]">
                 ...
                 <div className="flex space-x-3">
                     <FilterControl label="Registry Force" defaultLabel="ALL REGIONS" value={countryFilter} onChange={setCountryFilter} options={filterOptions.countries} disabled={!isSystemReady} />
                     <FilterControl label="Object Class" defaultLabel="ALL TYPES" value={typeFilter} onChange={setTypeFilter} options={filterOptions.types} disabled={!isSystemReady} />
                 </div>
                 <input
                    type="text"
                    value={searchQuery}
