...
import { RelayHealthStatus } from '../services/satelliteData';
...
const StatCard: React.FC<{ 
    ...
    className?: string;
}> = ({ title, value, valueColor = 'text-white', tooltipTitle, tooltipContent, tooltipFooter, className = "" }) => (
    <div className={`flex-1 p-4 bg-black rounded-sm border border-white/10 text-center cursor-help group relative transition-all duration-300 ${className}`}>
...
export const GlobalStatsBar: React.FC<GlobalStatsBarProps> = ({ alerts, satelliteCount, lastSyncTime, isSyncing, onSync, esStatus }) => {
    const [relayStatus, setRelayStatus] = useState<'LINKED' | 'STEALTH'>(relayService.getLinkStatus());

    useEffect(() => {
        const i = setInterval(() => {
            setRelayStatus(relayService.getLinkStatus());
        }, 2000);
        ...
        <div className="px-3 py-4 bg-black border-b border-white/10 z-10">
            <div className="flex items-center space-x-3">
                ...
                <StatCard
                    title="ES BACKEND"
                    value={
                        esStatus === 'healthy'  ? 'LINKED' :
                        esStatus === 'degraded' ? 'DEGRADED' :
                        esStatus === 'offline'  ? 'OFFLINE' :
                        relayStatus
                    }
                    valueColor={
                        esStatus === 'healthy'  ? 'text-green-500' :
                        esStatus === 'degraded' ? 'text-yellow-400' :
                        esStatus === 'offline'  ? 'text-red-500' :
                        relayStatus === 'LINKED' ? 'text-green-500' : 'text-gray-600'
                    }
                    tooltipTitle="Elasticsearch Health"
                    tooltipContent="Live connectivity check against the Elasticsearch backend via /v1/health. Polls every 60 seconds."
                    tooltipFooter={
                        esStatus === 'healthy'  ? 'Active pipeline to Elastic Cloud' :
                        esStatus === 'degraded' ? 'Auth issue — check API key' :
                        esStatus === 'offline'  ? 'Relay unreachable — cases queued locally' :
                        relayStatus === 'LINKED' ? 'Active Pipeline to Elastic Cloud' : 'Backend Unavailable'
                    }
                />
                <StatCard 
                    title="ANOMALY FEED" 
                    ...
                    tooltipContent="Number of active alerts requiring operator assessment."
                ...
                <StatCard 
                    ...
                    tooltipContent="The system prioritizes the most dangerous individual anomaly to ensure maximum operator awareness."
                ...
                <StatCard 
                    ...
                    tooltipContent="OrbitWatch periodically refreshes TLE data every 60 seconds."
