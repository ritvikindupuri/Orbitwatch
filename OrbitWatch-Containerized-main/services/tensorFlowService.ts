...
function extractFeatures(sat: RealSatellite): number[] | null {
    ...
    // Derive launch year from the COSPAR International Designator embedded in the TLE
    // (satrec.intldesg format: "YYNNNLLL", e.g. "01024A  " = 24th launch of 2001)
    ...
    } else if (sat.LAUNCH_DATE) {
        const parsed = parseInt(sat.LAUNCH_DATE.split('-')[0], 10);
        if (!isNaN(parsed)) launchYear = parsed;
    }
