...
// Low-level wrapper for IndexedDB to act as our NoSQL Store
class LocalDatabase {
    ...
    async saveSnapshot(catalog: RealSatellite[]): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                console.log(`[Database] Ingested snapshot of ${catalog.length} objects.`);
                resolve();
            };
    ...
    /**
     * Retrieves the "Massive Dataset" for ML Training.
     * Combines the latest snapshot with random historical samples to create a robust training set.
     */
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor && snapshotsCount < limit) {
                    ...
                    console.log(`[Database] Retrieved training set: ${massiveDataset.length} records from ${snapshotsCount} historical snapshots.`);
