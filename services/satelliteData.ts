...
/**
 * Fetches the satellite catalog directly from the centralized Elasticsearch backend.
 * The backend is responsible for synchronizing with Space-Track.
 */
export async function fetchElasticsearchCatalog(): Promise<RealSatellite[]> {
    try {
        const response = await fetch('/v1/satellites');
        relayService.handleResponseStatus(response.status);
        if (!response.ok) {
        ...
        console.warn("[SatelliteData] Elasticsearch fetch failed, using fallback snapshot.", error);
